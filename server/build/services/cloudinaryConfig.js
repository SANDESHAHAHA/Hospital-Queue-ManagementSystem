...
const configureCloudinary = () => {
    ...
    if (!cloudName || !apiKey || !apiSecret) {
        ...
        if (!cloudName)
            console.error('  - CLOUDINARY_CLOUD_NAME is not provided');
        if (!apiKey)
            console.error('  - CLOUDINARY_API_KEY is not provided');
        if (!apiSecret)
            console.error('  - CLOUDINARY_SECRET_KEY is not provided');
